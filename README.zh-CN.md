...
**先看第三行，再看第一行。** 每个任务的缓存未命中 token 都降了 22% 到 43%，这是这个 patch 直接控制的部分。但把它变成钱并不可靠。在实现任务上，精简后的 agent 反而多走了步数，请求数 4.4 对 5.4，输出多了 24%，把省下的大部分又吃回去了。它的逐次花费区间是重叠的，默认 $0.001749 到 $0.002996，dsh-lean $0.001854 到 $0.003034，也就是说在那个任务上 dsh-lean 的某一次运行可能比默认还贵。它留在表里是因为它是诚实的下限。本页早先的版本按每组 4 次运行给这一行写的是 18%，加到每组 7 次之后变成了 7%。
